...
/* 
  ...
  symbol = string
*/
Market.getFullOrderBookV2 = async function(symbol) {
...
Market.getFullOrderBook = async function(symbol) {
  const params = {symbol};
  let endpoint = '/api/v2/market/orderbook/level2'
...
/* 
  ...
  symbol = string
*/
Market.getFullOrderBookAtomicV2 = async function(symbol) {
...
Market.getFullOrderBook = async function(symbol) {
  const params = {symbol};
  let endpoint = '/api/v2/market/orderbook/level3'
  let url = this.baseURL + endpoint + this.formatQuery(params)
  let result = await axios.get(url, this.sign(endpoint, params, 'GET'))
  return result.data
}
